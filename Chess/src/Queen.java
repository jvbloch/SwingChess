...
public class Queen extends Piece{
	...
	public Queen(Player piecesPlayer, int lx, int ly) {
		super(piecesPlayer, lx, ly);
		
		this.setPointWorth(9);
		
		File imageCheck;
		if(this.getPlayer().name.equals("p1")){//is player 1
			
			image = new ImageIcon("queenW.png");
			//System.out.println(image.getDescription());
			
			imageCheck = new File("queenW.png");
		}
		else{
			image = new ImageIcon("queenB.png");
			//System.out.println(image.getDescription());
			
			imageCheck = new File("queenB.png");
		}
	...
	public ArrayList<Space> checkPossibleMoves(){//returns the piece's possible moves 
		...
		int examinedX = this.getLX()+1;
		int examinedY = this.getLY()-1;
		while(examinedX<8 && examinedY>-1){//add spaces going NorthEast
		...
		while(examinedX<8 && examinedY<8){//add spaces going SouthEast
			if(this.canMoveTo(examinedX, examinedY)){
				possibleSpaces.add(Chess.board.spaces[examinedY][examinedX]);
			}
			else{
		...
		examinedX = this.getLX()-1;
		examinedY = this.getLY()+1;
		while(examinedX>-1 && examinedY<8){//add spaces going SouthWest
			if(this.canMoveTo(examinedX, examinedY)){
				possibleSpaces.add(Chess.board.spaces[examinedY][examinedX]);
			}
			else{
		...
		examinedX = this.getLX()-1;
		examinedY = this.getLY()-1;
		while(examinedX>-1 && examinedY>-1){//add spaces going NorthWest
			if(this.canMoveTo(examinedX, examinedY)){
				possibleSpaces.add(Chess.board.spaces[examinedY][examinedX]);
			}
			else{
